...
fun TTSProviderConfigure(
    ...
    Column(
                        ...
                        DropdownMenuItem(
                            text = {
                                Text(
                                    when (providerClass) {
                                        ...
                                        TTSProviderSetting.CosyVoice::class -> "CosyVoice"
                            ...
                            onClick = {
                                ...
                                val newSetting = when (providerClass) {
                                    ...
                                    TTSProviderSetting.CosyVoice::class -> TTSProviderSetting.CosyVoice(
                                        id = setting.id,
                                        name = "CosyVoice TTS"
                                    )
        ...
        when (setting) {
            ...
            is TTSProviderSetting.CosyVoice -> CosyVoiceTTSConfiguration(setting, onValueChange)
        }
    }
}
...
@Composable
private fun CosyVoiceTTSConfiguration(
    setting: TTSProviderSetting.CosyVoice,
    onValueChange: (TTSProviderSetting) -> Unit
) {
    FormItem(
        label = { Text(stringResource(R.string.setting_tts_page_api_key)) },
    ...
    FormItem(
        ...
        OutlinedTextField(
            ...
            modifier = Modifier.fillMaxWidth(),
            placeholder = { Text("zh_female_qingxin (输入克隆音色ID)") }
        )
    }
            ...
            ExposedDropdownMenu(
                ...
                languages.forEach { language ->
                    DropdownMenuItem(
                        ...
                        onClick = {
                            languageExpanded = false
                            onValueChange(setting.copy(language = language))
                        }
                    )
                }
            }
    ...
    val sampleRates = listOf(16000, 24000, 48000)
            ...
            OutlinedTextField(
                ...
                onValueChange = { newRate ->
                    newRate.toIntOrNull()?.let {
                        onValueChange(setting.copy(sampleRate = it))
                    }
                },
                modifier = Modifier
                    .fillMaxWidth()
    ...
    var emotionExpanded by remember { mutableStateOf(false) }
    val emotions = listOf("neutral", "happy", "sad", "angry", "fearful", "excited")
    ...
    FormItem(
        label = { Text("Pitch") },
        description = { Text("Pitch adjustment (-12 to 12 semitones)") }
    ) {
        OutlinedNumberInput(
            value = setting.pitch,
            onValueChange = { newPitch ->
                if (newPitch in -12f..12f) {
                    onValueChange(setting.copy(pitch = newPitch))
                }
            },
            modifier = Modifier.fillMaxWidth(),
