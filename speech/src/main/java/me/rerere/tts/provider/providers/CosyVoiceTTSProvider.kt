...
class CosyVoiceTTSProvider : TTSProvider<TTSProviderSetting.CosyVoice> {
    ...
    override fun generateSpeech(
        ...
        val httpRequest = Request.Builder()
            .url("${providerSetting.baseUrl}/services/aigc/text_to_speech/speech_synthesis")
        ...
        if (!response.isSuccessful) {
            ...
            Log.e(TAG, "CosyVoice TTS request failed: ${response.code} ${response.message}, body: $errorBody")
            throw Exception("CosyVoice TTS request failed: ${response.code} ${response.message}")
        }
